...
const HOME_COMPANY_IDS = ['co1', 'co2', 'co3', 'co4', 'co5', 'co6'];
...
function Hero({ onSearch, onQuickTag }) {
  ...
  const homeCompanies   = mockCompanies.filter((c) => HOME_COMPANY_IDS.includes(c.id));
    ...
    <section style={hero.section}>
      ...
      <div style={hero.content}>
        ...
        <div style={hero.stats}>
          {[
            { val: activeJobs,             label: "Active Jobs"  },
            { val: homeCompanies.length,   label: "Companies"    },
...
function JobCard({ job, saved, onToggleSave }) {
  ...
  const logo = logoMap[job.companyId];

  return (
    <div
  style={{
    ...jc.card,
    ...(job.featured ? jc.featured : {}),
    ...(hov ? jc.hover : {}),
  }}
...
>
      ...
      <div style={jc.top}>
        {/*  */}
        <div style={{ ...jc.logo, background: logo ? "#F6F5F2" : (co?.color ?? "#F6F5F2"), color: logo ? "inherit" : "#fff" }}>
          {logo
            ? <img src={logo} alt={co?.name} style={{ width: "38px", height: "38px", objectFit: "contain" }} />
      ...
      <div style={jc.footer}>
        ...
        {/*  */}
        <span style={jc.meta}>Posted {timeAgo(job.postedAt)}</span>
      </div>
    </div>
  );
}
...
function FeaturedJobs({ savedJobs, onToggleSave, onViewAll }) {
  ...
  const featured = mockJobs.filter(
    (j) => j.featured && j.status === "open" && HOME_COMPANY_IDS.includes(j.companyId)
  );
  return (
  ...
  return (
    <section style={{ marginBottom: "40px" }}>
      ...
      <div style={sec.coGrid}>
        {homeCompanies.map((co) => <CompanyCard key={co.id} co={co} />)}
      </div>
    </section>
  );
}
...
>
      {/*  */}
      <div style={{ ...sec.coLogo, background: logo ? "#F6F5F2" : (co.color ?? "#F6F5F2"), color: logo ? "inherit" : "#fff" }}>
        {logo
          ? <img src={logo} alt={co.name} style={{ width: "55px", height: "55px", objectFit: "contain" }} />
          : <span>{co.initials}</span>
        }
      </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh", background: "#F6F5F2", fontFamily: "'Instrument Sans',sans-serif" }}>
      ...
      <Navbar
        ...
        onLogout={() => { setUser(null); routerNavigate("/"); }}
      />
